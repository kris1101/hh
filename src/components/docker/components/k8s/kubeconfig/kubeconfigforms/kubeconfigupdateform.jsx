...
const ClusterUpdateForm = Form.create()(
  class extends React.Component {
    ...
    render() {
      const { confirmLoading, visible, onCancel, onCreate, clusterinfo, form } = this.props;
        ...
        >
          <Form>
            <FormItem label="集群名称"  labelCol={{span: 5}}  wrapperCol={{ span: 19 }} hasFeedback={true}>
              {getFieldDecorator('clustername', { initialValue: clusterinfo.cluster_name,
              rules: [{ required: true, validator: (rule, value, callback) => {this.checkprojectexists(rule, value, callback, clusterinfo.cluster_name)}}],
              })(
                <Input disabled={confirmLoading} />
              )}
            </FormItem>
            <FormItem label="所属地区"  labelCol={{span: 5}}  wrapperCol={{ span: 19 }} hasFeedback={true}>
              {getFieldDecorator('clusterzone', {initialValue: clusterinfo.cluster_address,
            ...
            <FormItem label="dns类型"  labelCol={{span: 5}}  wrapperCol={{ span: 19  }}>
              {getFieldDecorator('dns_type', {
                initialValue: clusterinfo.dns_type, rules: [{required: true}],
              })(
